...
class MainWindow:
    ...
    def _create_mode_selection(self):
        ...
        ttk.Label(self.window, text="Mode:", anchor="w").grid(
            row=0, column=0, sticky="w", pady=8, padx=(10, 5))
        
        self.combo_mode = ttk.Combobox(self.window, values=["extract", "info"], 
                                       state="readonly", width=15, textvariable=self.var_mode)
        ...
        self.btn_reset = ttk.Button(right_frame, text="Reset", 
                                   command=self._reset_to_default, 
                                   style="warning.TButton", width=8)
        self.btn_reset.pack(side="left", padx=(0, 5))
        
        self.btn_about = ttk.Button(right_frame, text="About", 
    ...
    def _create_advanced_options(self):
        ...
        self.advanced_frame = ttk.Labelframe(self.window, text="Advanced Options", padding=10)
    ...
    def _create_advanced_widgets(self):
        ...
        # Info mode widgets
        self.combo_sortby = ttk.Combobox(self.advanced_frame, values=SORT_OPTIONS,
    ...
    def _create_action_buttons(self):
        ...
        self.btn_advanced = ttk.Button(center_frame, text="Show Advanced Options ▼",
                                      command=self._toggle_advanced, 
                                      style="info.TButton", width=25)
        self.btn_advanced.pack(side="left", padx=(0, 10))
        
        ttk.Button(center_frame, text="▶ Run", command=self._run_repkg,
                  style="success.TButton", width=20).pack(side="left")
    
    def _create_output_console(self):
        """Create output console section."""
        output_frame = ttk.Labelframe(self.window, text="Console Output", padding=10)
    ...
    def _setup_event_bindings(self):
        ...
        # Combobox events
        self.combo_sortby.bind("<<ComboboxSelected>>", self._on_config_change)
    ...
    def _create_extract_options(self, start_row):
        ...
        ttk.Label(self.advanced_frame, 
                 text="Only extract files with specified extensions (delimited by comma \",\"):").grid(
                 row=row, column=0, sticky="w", pady=5)
        self.entry_exts_only.grid(row=row, column=1, pady=5)
        row += 1
        
        ttk.Label(self.advanced_frame,
                 text="Don't extract files with specified extensions (delimited by comma \",\"):").grid(
                 row=row, column=0, sticky="w", pady=5)
        self.entry_exts_ignore.grid(row=row, column=1, pady=5)
        row += 1
        
        # Extract options checkboxes
        for var_name, description in EXTRACT_OPTIONS:
            var = getattr(self, f"var_{var_name}")
            ttk.Checkbutton(self.advanced_frame, text=description, variable=var).grid(
                row=row, column=0, sticky="w", pady=3)
            row += 1
        
        # Log option
        ttk.Checkbutton(self.advanced_frame, text="Save output to log file", 
    ...
    def _create_info_options(self, start_row):
        ...
        # Sort by
        ttk.Label(self.advanced_frame, text="Sort by:").grid(row=row, column=0, sticky="w", pady=5)
        self.combo_sortby.grid(row=row, column=1, sticky="w", pady=5)
        row += 1
        
        # Info options checkboxes
        for var_name, description in INFO_OPTIONS:
            var = getattr(self, f"var_{var_name}")
            ttk.Checkbutton(self.advanced_frame, text=description, variable=var).grid(
                row=row, column=0, sticky="w", pady=3)
            row += 1
        ...
        # Project info entry
        ttk.Label(self.advanced_frame,
                 text="Keys to dump from project.json (delimit using comma) (* for all):").grid(
                 row=row, column=0, sticky="w", pady=5)
        self.entry_projectinfo.grid(row=row, column=1, pady=5)
        row += 1
        
        # Title filter entry
        ttk.Label(self.advanced_frame, text="Title filter:").grid(row=row, column=0, sticky="w", pady=5)
    ...
    def _toggle_advanced(self):
        ...
        if self.advanced_frame.winfo_ismapped():
            ...
            self.btn_advanced.config(text="Show Advanced Options ▼")
        else:
            self.advanced_frame.grid()
            self.btn_advanced.config(text="Hide Advanced Options ▲")
